        ...
        if (operacion.equals("salir")) {
            ...
            try {
                st = conexion.prepareStatement(
                        "select count(usuario),nombre_persona from persona where usuario= ? and contrasenia=SHA2(?,?) ");
                st.setString(1, usuario);
                st.setString(2, usuario);
                st.setString(3, password);
                rs = st.executeQuery( );
                out.println("Resultado" + st);
                rs.next( );
            ...
            String usuario=request.getParameter("usuario");
            String password=request.getParameter("password");
            st = conexion.prepareStatement("INSERT INTO persona(" +
                    "nombre_persona, edad_persona, sexo_persona, id_ocupacion, " +
                    "fecha_nac, usuario, contrasenia) " +
                    "VALUES (?, ? , ?, ? ,?, ?, SHA2(?,256))" );
            ...
            st = conexion.prepareStatement(
                    "UPDATE persona SET nombre_persona=?,edad_persona=?," +
                            "sexo_persona=?,id_ocupacion=?,fecha_nac=?,usuario =? " +
                            "WHERE id_persona=?");
            st.setString(1,nombre);
            st.setInt(2, edad);
            st.setString(3, sexo);
